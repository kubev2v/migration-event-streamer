...
func AssessmentCreatedProcessor(_ context.Context, event entity.Event[plannerEvents.AssessmentEventPayload]) (entity.AssessmentCreatedResult, error) {
...
func AssessmentDeletedProcessor(_ context.Context, event entity.Event[plannerEvents.AssessmentEventPayload]) (entity.AssessmentDeletedResult, error) {
	zap.S().Infow("processing assessment deleted event",
		"assessment_id", event.Payload.Assessment.ID)

	return entity.AssessmentDeletedResult{
		DeletedID: event.Payload.Assessment.ID,
		DeletedAt: event.Payload.Assessment.DeletedAt.Format(time.RFC3339),
	}, nil
}
