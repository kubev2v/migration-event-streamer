...
func (c *ContainerInfraManager) StartKafka() error {
	...
	_, err := c.runner.StartContainer(
		NewContainerConfig(kafkaContainerName, kafkaImage).
			...
			WithEnvVar("KAFKA_LISTENERS", "PLAINTEXT://0.0.0.0:9092,CONTROLLER://0.0.0.0:9093").
			WithEnvVar("KAFKA_ADVERTISED_LISTENERS", "PLAINTEXT://localhost:9092").
			WithEnvVar("KAFKA_CONTROLLER_LISTENER_NAMES", "CONTROLLER").
			WithEnvVar("KAFKA_LISTENER_SECURITY_PROTOCOL_MAP", "CONTROLLER:PLAINTEXT,PLAINTEXT:PLAINTEXT").
